...
class H36mSkeletonHelper(object):
    ...
    def _pose2euler(self, pose, header):
        channel = []
        quats = {}
        eulers = {}
        stack = [header.root]
    ...
    def pose2euler(self, poses_3d, header=None):
        if not header:
            header = self.get_bvh_header(poses_3d)
        
        joint_names = list(self.keypoint2index.keys())
        frames = {n: np.zeros((len(poses_3d), 3)) for n in joint_names}
        for frame, pose in enumerate(poses_3d):
            channels = self._pose2euler(pose, header)
            channels = np.array(channels).reshape(-1, 3)
            for i, c in enumerate(channels):
                joint = joint_names[i]
                frames[joint][frame] = c
        return frames
    ...
    def poses2bvh(self, poses_3d, header=None, output_file=None):
        ...
        channels = []
        for frame, pose in enumerate(poses_3d):
            channels.append(self._pose2euler(pose, header))
